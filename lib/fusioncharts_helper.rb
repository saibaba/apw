...
@@arr_FCColors[19] = "669900" #Shade of green
  ...
  def render_chart(chart_swf,str_url,str_xml,chart_id,chart_width,chart_height,debug_mode,register_with_js,&block)
    ...
    raw_concat("\t\t<!-- START Script Block for Chart "+chart_id+" -->\n\t\t")
    raw_concat(content_tag("div","\n\t\t\t\tChart.\n\t\t",{:id=>chart_id+"Div",:align=>"center"}))
    raw_concat("\n\t\t<script type='text/javascript'>\n")
    
    raw_concat("\t\t\t\tvar chart_"+chart_id+"=new FusionCharts('"+chart_swf+"','"+chart_id+"',"+chart_width+","+chart_height+","+debug_mode_num+","+register_with_js_num+");\n")
    
    if str_xml==""
      raw_concat("\t\t\t\t<!-- Set the dataURL of the chart -->\n")
      raw_concat("\t\t\t\tchart_"+chart_id+".setDataURL(\""+str_url+"\");\n")
      logger.info("The method used is setDataURL.The URL is " + str_url)
    else
      raw_concat("\t\t\t\t<!-- Provide entire XML data using DataXML method -->\n")
      #concat("\t\t\t\tchart_"+chart_id+".setDataXML(\""+str_xml+"\");\n")
      raw_concat("\t\t\t\t")
      raw_concat('chart_'+chart_id+'.setDataXML(\''+str_xml+'\');')
      raw_concat("\n")
      logger.info("The method used is setDataXML.The XML is " + str_xml)
    end
    
    raw_concat("\t\t\t\t<!-- Finally render the chart. -->\n")
    raw_concat("\t\t\t\tchart_"+chart_id+".render('"+chart_id+"Div');\n")
    raw_concat("\t\t</script>\n")
    raw_concat("\t\t<!-- END Script Block for Chart "+chart_id+" -->\n")
